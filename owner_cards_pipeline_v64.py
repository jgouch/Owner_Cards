#!/usr/bin/env python3
"""Owner Card Pipeline — v64 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v64.py
  or
  python3 owner_cards_pipeline_v64.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def _auto_rotate_by_header_score(pil_img: Image.Image, header_ratio: float = 0.35) -> tuple[Image.Image, dict]:
    ...
    for angle in (0, 90, 180, 270):
        try:
            ...
            # v64: tie-break orientation when OCR scores are close: prefer landscape (w/h)
            ratio = (w / max(h, 1))
...
def extract_phone_fields(full_text: str, lines: List[str]) -> Dict[str, object]:
    ...
    # v64: guard against false 10-digit phones formed by ZIP tail + 7-digit local number
    ...
    for raw in matches:
        ...
        # If candidate is 10 digits and starts with the last 3 digits of a ZIP that appears immediately before a 7-digit local number, treat as fused ZIP+phone.
...
def parse_best_address(lines: List[str]) -> Dict:
    ...
    for i, line in enumerate(lines):
        ...
        if z and st:
            ...
            # v64: if the street line ends with a known city (e.g., '... Fairview') and the next line is only state+ZIP, split it
            city_from_street = ''
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    # v64: FaCTS load mode tracking (not_provided vs loaded vs failed)
    ...
    with pd.ExcelWriter(tmp_path, engine=engine) as xw:
        ...
        # v64: convenience sheet with only rows that need human review
        try:
