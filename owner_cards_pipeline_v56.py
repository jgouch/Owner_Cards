#!/usr/bin/env python3
"""
Owner Card Pipeline — v56 (Stable Release)

Fixes from v55:
- Syntax: Fixed indentation errors in header parsing.
- Logic: Defined missing helper functions (extract_phone, is_gibberish, etc.).
- OCR: Now correctly applies deskewing to the image *before* OCR (previously computed but ignored).
- Safety: Added robust .get() access for address dictionaries to prevent KeyErrors.
- Architecture: Consolidated Text Layer and OCR Fallback to use the same extraction logic.
...
def extract_phone(text: str) -> Tuple[str, bool]:
    """Wrapper for backward compatibility in process_page"""
    fields = extract_phone_fields(text, [])
    return fields["PhoneNormalized"], fields["PhoneHasAreaCode"]
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str]) -> Tuple[Dict, List[Dict], bool]:
    ...
    # DESKEW FIRST (Fixes v55 bug where deskew was computed but not used)
    orig_bgr = cv2.cvtColor(np.array(pil_orig), cv2.COLOR_RGB2BGR)
    deskewed_bgr = deskew_bgr(orig_bgr)
    pil_deskewed = Image.fromarray(cv2.cvtColor(deskewed_bgr, cv2.COLOR_BGR2RGB))
    ...
    # If still missing, try Deep Upscale
    if "[MISSING" in p:
    ...
    return ({
         "OwnerName_Raw": f"{p} {s}", "PrimaryOwnerName": p, "SecondaryOwnerName": s,
         "LastName": last, "Phone": "", "Street": addr.get('Street',''), "City": addr.get('City',''),
         "State": addr.get('State',''), "ZIP": addr.get('ZIP',''), "AddressRaw": addr.get('AddressRaw',''),
...
def process_dataset(pdf_path, out_path, dpi=300):
    ...
    except: page_count = len(PdfReader(pdf_path).pages)
    ...
    for col in df.columns:
        df[col] = df[col].apply(lambda x: excel_safe_text(x) if isinstance(x, str) else x)
