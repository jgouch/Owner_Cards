#!/usr/bin/env python3
"""Owner Card Pipeline — v60 (Py3.14-safe + Accuracy fixes)
...
Run:
  python3 owner_cards_pipeline_v60.py
  or
  python3 owner_cards_pipeline_v60.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
def dewarp_perspective(img_bgr: np.ndarray) -> np.ndarray:
    """Best-effort perspective correction using largest 4-point contour. No-op if not found."""
    try:
        gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
...
def ocr_items_with_strike(pdf_path: str, page_index: int, dpi: int, strike_segs: List[Tuple[int, int, int, int]]) -> List[Dict]:
    ...
    img_bgr = deskew_bgr(img_bgr)
    pil_deskew = Image.fromarray(cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB))
    return ocr_items_with_strike_from_image(pil_deskew, strike_segs=strike_segs, y_min_frac=ITEMS_START_RATIO)
...
def try_kraken_text(pil_img: Image.Image, model_path: str = '', kraken_bin: str = 'kraken', kraken_python: str = '') -> str:
    """Last-resort OCR using Kraken via CLI. Requires kraken CLI + a recognition .mlmodel.

    If kraken_python is provided (python 3.11 venv), we derive the kraken executable from it
    (same folder as python). This lets the main pipeline run on Python 3.14 while Kraken runs in 3.11.
    """
    if not model_path:
        return ''
    import tempfile
...
# --- Header-specific scoring & OCR ensemble (name/address live in top 35%) ---
...
def ocr_header_ensemble(pil_page: Image.Image, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True) -> tuple[str, dict]:
    """Run multiple OCR strategies on the header region only and return (best_text, meta)."""
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', alt_ocr_order: str = 'ocrmac_then_kraken', kraken_python: str = '', allow_livetext: bool = True) -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            if strike_segs and strike_pil is not None:
                ...
                items = [] if is_interment else ocr_items_with_strike_from_image(strike_pil, strike_segs=strike_segs, y_min_frac=ITEMS_START_RATIO)
            else:
    ...
    # --- v60: Header-targeted OCR ensemble (top region) ---
        ...
        try:
            ...
            if alt_ocr_order == 'none':
                ...
                if kraken_model:
                    ...
                    alt_text = try_kraken_text(pil_alt, model_path=kraken_model, kraken_bin=kraken_bin, kraken_python=kraken_python)
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', alt_ocr_order: str = 'ocrmac_then_kraken', kraken_python: str = '', allow_livetext: bool = True):
    ...
    for p in tqdm(range(page_count), desc=f"Scanning {filename}", unit="page"):
        owner_data, items_data, is_interment = process_page(pdf_path, p, dpi, target_char, reader=reader, kraken_model=kraken_model, kraken_bin=kraken_bin, alt_ocr_order=alt_ocr_order, kraken_python=kraken_python, allow_livetext=allow_livetext)
    ...
    stats = pd.DataFrame([{
        ...
        "OCRMAC_UsedPages": int(owners_master.get("TextSource","").astype(str).str.contains("OCRMAC", na=False).sum()) if not owners_master.empty else 0,
        "KRAKEN_UsedPages": int(owners_master.get("TextSource","").astype(str).str.contains("KRAKEN", na=False).sum()) if not owners_master.empty else 0,
        "LIVETEXT_UsedPages": int(owners_master.get("TextSource","").astype(str).str.contains("LIVETEXT", na=False).sum()) if not owners_master.empty else 0,
...
def main():
    ...
    ap.add_argument("--kraken-python", default="", help="Path to python in Kraken (Py3.11) venv; if set, use venv kraken executable")
    ap.add_argument("--no-livetext", action="store_true", help="Disable ocrmac LiveText backend even if available")
    ...
    if args.pdf:
        ...
        process_dataset(args.pdf, out, args.dpi, kraken_model=args.kraken_model, kraken_bin=args.kraken_bin, alt_ocr_order=args.alt_ocr, kraken_python=args.kraken_python, allow_livetext=(not args.no_livetext))
        return
    ...
    for pdf_path in pdf_files:
        ...
        process_dataset(pdf_path, out_path, dpi=args.dpi, kraken_model=args.kraken_model, kraken_bin=args.kraken_bin, alt_ocr_order=args.alt_ocr, kraken_python=args.kraken_python, allow_livetext=(not args.no_livetext))
