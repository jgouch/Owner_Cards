#!/usr/bin/env python3
"""Owner Card Pipeline — v70.5 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v70_5.py
  or
  python3 owner_cards_pipeline_v70_5.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
# --- OCR safety: prevent hangs (v70.5) ---
...
# --- Snapshot location: keep with script/batch folder (v70.5) ---
...
def enrich_owner_with_review(owner: Dict, items: List[Dict], pdf_path: str, page_num: int, dpi: int, pil_page: Optional[Image.Image] = None, facts_sections: Optional[List[str]] = None, facts_mode: str = '') -> Dict:
    ...
    # v70.5: Only validate FaCTS sections on lines that actually look like they contain garden/lot/sec/space patterns.
...
# -----------------------------
# v70.5 PROPERTY HEURISTICS (garden + coordinate OCR fuzz)
# -----------------------------
DEFAULT_GARDENS = [
    "GOOD SHEPHERD",
    "EVERLASTING LIFE",
...
# --- Structural / colon-style property detection (v70.5 fix) ---
    ...
    for tag, img_v in variants:
        ...
        for psm in psm_list:
            ...
            if sc > best_score:
                ...
                # v70.5: stop early when header OCR is already very strong
                if best_score >= 95:
                    break
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            # v70.5: salvage State/ZIP from anywhere on page when text-layer address parse is incomplete
            try:
