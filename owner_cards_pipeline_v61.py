#!/usr/bin/env python3
"""Owner Card Pipeline — v61 (Py3.14-safe + Accuracy fixes)
...
def try_kraken_text(pil_img: Image.Image, model_path: str = '', kraken_bin: str = 'kraken', kraken_python: str = '') -> str:
    """Last-resort OCR using Kraken via CLI. Requires kraken CLI + .mlmodel.
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True) -> Tuple[Dict, List[Dict], bool]:
    ...
    # --- v61: Header-targeted OCR ensemble (top region) ---
    ...
    # Visual Sniper: snapshot if name missing
    if ('[MISSING' in (p or '')):
        try:
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True):
    ...
    for p in tqdm(range(page_count), desc=f"Scanning {filename}", unit="page"):
        owner_data, items_data, is_interment = process_page(pdf_path, p, dpi, target_char, reader=reader, kraken_model=kraken_model, kraken_bin=kraken_bin, kraken_python=kraken_python, allow_livetext=allow_livetext)
...
def main():
    ap = argparse.ArgumentParser(description="Owner Cards OCR pipeline v61. Drag-drop: drag this .py then drag the PDF then press Enter.")
    ...
    if args.pdf:
        ...
        process_dataset(args.pdf, out, args.dpi, kraken_model=args.kraken_model, kraken_bin=args.kraken_bin, kraken_python=args.kraken_python, allow_livetext=(not args.no_livetext))
        return
    ...
    for pdf_path in pdf_files:
        ...
        process_dataset(pdf_path, out_path, dpi=args.dpi, kraken_model=args.kraken_model, kraken_bin=args.kraken_bin, kraken_python=args.kraken_python, allow_livetext=(not args.no_livetext))
