#!/usr/bin/env python3
"""Owner Card Pipeline — v72.1 (Drag-drop FaCTS Auto-detect + v72 fixes)
...
def match_facts_section_from_line(line_text: str, facts_sections: List[str], facts_index: Optional[List[Tuple[str, str]]] = None) -> Tuple[str, float, str]:
    ...
    if not line_text or not facts_sections:
        return ('', 0.0, '')
    # v72: dimension tokens like 44:X:13 are marker sizes, not sections
    ...
    # 1.2) Extract alphabetic segments from colon/slash chains (e.g., 153:Sp.1-2:C:Gethsemane)
    ...
    if alpha_segs:
        ...
        best_ratio = 0.0
        best_method = ''
        for cand2 in alpha_segs:
            cand2n = normalize_section_name(cand2)
            for sec, nsec in idx:
                # containment for family variants (e.g., CHAPEL HILL matches CHAPEL HILL - EAST)
                if (cand2n in nsec) or (nsec in cand2n):
                    r = 0.88
                    method = 'colon_family'
                else:
                    r = difflib.SequenceMatcher(None, cand2n, nsec).ratio()
                    method = 'colon_fuzzy'
                if r > best_ratio:
                    best_ratio = r
                    best_sec = sec
                    best_method = method
        if best_sec and best_ratio >= 0.74:
            return (best_sec, float(best_ratio), best_method)
...
def main():
    ...
    positional = _[:]
 # v72.1: allow drag-drop of FaCTS inventory without requiring --facts
 if (not getattr(args, 'facts', '')) and positional:
  for tok in positional:
   try:
    if isinstance(tok, str) and tok.lower().endswith(('.xlsx','.xlsm','.xls')) and os.path.exists(tok):
     args.facts = tok
     break
   except Exception:
    continue
