...
def _auto_pick_pdf(script_dir: str) -> str:
    """Pick a default PDF with minimal/no prompting.

    Order:
      1) If env OWNERCARDS_PDF set and exists, use it.
      2) If exactly one PDF in CWD, use it.
      3) Prefer PDFs matching '* (all).pdf' (or containing '(all)') newest first.
      4) Otherwise, if exactly one PDF in script_dir, use it.
      5) If multiple remain, return '' (caller may prompt).
    """
...
def _auto_pick_kraken_model() -> str:
    """Pick a default Kraken model path.

    Order:
      1) OWNERCARDS_KRAKEN_MODEL env var
      2) Newest McCATMuS*.mlmodel under ~/Library/Application Support/htrmopo
      3) Newest *.mlmodel under same directory
    """
...
def _prompt_pick(items, prompt='Choose:'):
...
def try_kraken_text(pil_img: Image.Image, model_path: str = "", kraken_bin: str = "kraken") -> str:
    ...
    Requires kraken installed (CLI on PATH) and a recognition model path (.mlmodel).
    """
    if not model_path:
        return ""
    import tempfile
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', alt_ocr_order: str = 'ocrmac_then_kraken') -> Tuple[Dict, List[Dict], bool]:
    ...
    )[0]    # ---- LAST RESORT: Deep OCR Retry (2x upscale) ----
    if best_score < 35:
        try:
            scale = 2.0
    ...
    if best_score < 35:
        try:
            ...
            alt_text = ''
            # Order: ocrmac_then_kraken (A) or kraken_then_ocrmac
...
def main():
    ...
    # --- Drag/drop friendly: allow positional PDF/out, env defaults, and auto-detection ---
    positional = _[:]
    # If user dragged a PDF after the script (no flags), it will appear in positional
    if (not args.pdf) and positional:
        for tok in positional:
            if isinstance(tok, str) and tok.lower().endswith('.pdf') and os.path.exists(tok):
                args.pdf = tok
                break
    # Auto-pick a PDF if none provided
    script_dir = os.path.dirname(os.path.abspath(__file__))
    if not args.pdf:
        args.pdf = _auto_pick_pdf(script_dir)
    # If still none, prompt once (only when ambiguous / nothing found)
    if not args.pdf:
        candidates = sorted(glob.glob(os.path.join(os.getcwd(), '*.pdf'))) + sorted(glob.glob(os.path.join(script_dir, '*.pdf')))
        # de-dup
        candidates = list(dict.fromkeys(candidates))
        if candidates:
            picked = _prompt_pick(candidates, prompt='Select PDF number (Enter to cancel):')
            if picked:
                args.pdf = picked
    # Default output path
    if args.pdf and (not args.out):
        args.out = _default_out_path(args.pdf)
    # Auto-pick Kraken model if not supplied (only used if alt OCR includes kraken)
