#!/usr/bin/env python3
"""Owner Card Pipeline — v68 (FaCTS Section Validation + Full Snapshots + NeedsReviewNotes)
...
Run:
  python3 owner_cards_pipeline_v68.py
  or
  python3 owner_cards_pipeline_v68.py --pdf "B (all).pdf" --out "OwnerCards_B_Output.xlsx"
...
# v67: base directory for snapshot outputs (set to PDF folder during processing)
SNAPSHOT_BASE_DIR = ""
...
def preprocess_ghost(pil_img: Image.Image) -> Image.Image:
    ...
    bin_img = cv2.erode(bin_img, kernel, iterations=1)  # v67: thicken faint ink
...
def preprocess_high_contrast(pil_img: Image.Image) -> Image.Image:
    """v67: high-contrast preprocessor for yellowed paper/light typing."""
    try:
        img_bgr = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)
...
def normalize_for_ocr(pil_img: Image.Image) -> Tuple[Image.Image, Dict]:
    """v67: apply OSD auto-rotation + deskew before OCR. Returns (image, meta)."""
...
def should_apply_osd_deskew(pdf_text: str, text_layer_ok: bool, primary_name: str, addr: Dict, phone_fields: Dict, items: List[Dict], header_ocr_meta: Optional[Dict] = None) -> Tuple[bool, str]:
    """v67: decide whether to run OSD+deskew normalization BEFORE OCR."""
...
def should_scan_strikes(text: str) -> bool:
    """v67: Only run strike detection on pages likely to contain cancellations/transfers (speed)."""
    u = (text or '').upper()
    for kw in ('TRANSFER', 'TRANSFERRED', 'CANCEL', 'CANCELLED', 'VOID', 'REFUND', 'SOLD', 'REWRITE', 'NO LONGER', 'GIVEN TO'):
        if kw in u:
            return True
    return False
...
# v68: garden/property pattern helpers can be loaded from FaCTS (Section list) for better coverage.
# Default garden tokens (used when FaCTS not provided or fails to load)
GARDEN_TOKENS_DEFAULT = [
    "PRAYER", "LAST SUPPER", "GETHSEMANE",
    "CHAPEL HILL", "CHAPEL HILL EAST", "CHAPEL HILL WEST",
    "PEACE", "FAITH", "SERENITY", "FOUNTAIN",
    "EVERLASTING LIFE", "GOOD SHEPHERD", "CROSS", "ATONEMENT",
    "GARDEN OF GRACE", "GARDEN OF PEACE",
    "LAKEVIEW", "LAKEVIEW ESTATES",
    "MOUNTAIN VIEW", "MT VIEW", "BELL TOWER", "TRANQUILITY", "LAKESIDE",
]

# Active garden tokens (informational) + regex for raw text fallback
GARDEN_TOKENS = list(GARDEN_TOKENS_DEFAULT)
GARDEN_RE = re.compile(r"\b(?:" + "|".join([re.escape(x) for x in sorted(GARDEN_TOKENS, key=len, reverse=True)]) + r")\b", re.IGNORECASE)

# Normalized garden regex (preferred): built from normalize_section_name(FaCTS Section values)
GARDEN_NORM_RE: Optional[re.Pattern] = None


def init_garden_tokens_from_facts(facts_sections: Optional[List[str]]) -> None:
    """Initialize garden detection from FaCTS Section list.

    We compile a regex over normalized section names and test against normalize_section_name(line).
    This improves matching for hyphens/punctuation/spacing variations.
    """
    global GARDEN_TOKENS, GARDEN_RE, GARDEN_NORM_RE
    ...
    if not facts_sections:
        return

    norm_list = []
    seen = set()
    for s in facts_sections:
        ns = normalize_section_name(s)
        if not ns:
            continue
        if ns in ("SECTION", "GARDEN"):
            continue
        if len(ns) < 3:
            continue
        if ns in seen:
    ...
    # Keep human-readable tokens for reference
    GARDEN_TOKENS = list(facts_sections)

    parts = [re.escape(x) for x in sorted(norm_list, key=len, reverse=True)]
    try:
        GARDEN_NORM_RE = re.compile(r"\b(?:" + "|".join(parts) + r")\b")
    except Exception:
        GARDEN_NORM_RE = None


# plot codes like 308A 1, 92C 3, 2028 1, 133:C:3-4
PLOT_CODE_RE = re.compile(r"(?:\b\d{1,4}[A-Z]\b(?:\s*\d{1,2})?|\b\d{1,4}\b\s*[:\-]\s*[A-Z]\b|\b[A-Z]\s*[:\-]\s*\d{1,4}\b)", re.IGNORECASE)
# colon/compact location family
COLON_LOC_RE = re.compile(r'LOT\s*\"?\d+|SEC\s*\"?[A-Z]|SP\s*\"?\d', re.IGNORECASE)
# partial-space cancellation patterns (e.g., X 2 over a space number)
X_SPACE_RE = re.compile(r"\bX\s*([0-9]{1,2})\b", re.IGNORECASE)
SPACE_RANGE_RE = re.compile(r"\bSP\.?\s*([0-9\-\s]+)\b", re.IGNORECASE)
...
def group_text_lines_from_ocr(data: Dict, min_conf: int = 30) -> List[Dict]:
    n = len(data.get("text", []))
    words = []
    for i in range(n):
        txt = data["text"][i]
...
def line_is_struck(line_bbox: Tuple[int, int, int, int], strike_segs: List[Tuple[int, int, int, int]]) -> bool:
    """v67: consider a line struck only when a strike segment overlaps a meaningful portion of its width."""
    x1, y1, x2, y2 = line_bbox
    w = max(1, x2 - x1)
    for sx1, sy1, sx2, sy2 in (strike_segs or []):
        if not (y1 - 2 <= sy1 <= y2 + 2):
            continue
        overlap = max(0, min(x2, sx2) - max(x1, sx1))
        if (overlap / w) >= 0.35:
            return True
    return False
...
def ocr_items_with_strike_from_image(pil_img: Image.Image, strike_segs: List[Tuple[int, int, int, int]], y_min_frac: float = ITEMS_START_RATIO) -> List[Dict]:
    ...
    raw_lines_a = group_text_lines_from_ocr(d_std, min_conf=10)
    raw_lines_b = group_text_lines_from_ocr(d_clahe, min_conf=10)
    items: List[Dict] = []
    seen = set()
...
def get_header_candidate(lines: List[str], addr_idx: Optional[int], target_char: Optional[str], aggressive: bool) -> List[str]:
    """v67: score multiple candidate name lines (avoid crossed-out owners / transfer trap)."""
    top = lines[:40]
    search_lines = top[:addr_idx] if (addr_idx is not None and addr_idx > 0) else top[:12]
    candidates = []
    for ln in search_lines:
        ...
        if ',' in ln_clean:
            sc += 25
        if target_char and u.startswith(target_char.upper()):
            sc += 15
        if re.search(r"\b(TRANSFER|TRANSFERRED|CANCEL|CANCELLED|VOID|SOLD|REWRITE)\b", u):
            sc -= 40
        if len(u.split()) == 1:
            sc -= 10
        candidates.append((sc, ln_clean))
...
def remove_xd_spaces(line: str) -> str:
    """v67: remove only X'd space numbers from a Sp. range (partial cancellation)."""
    if not line:
        return line
    m = SPACE_RANGE_RE.search(line)
    if not m:
        return line
    spaces_txt = re.sub(r"\s+", "", m.group(1))
    parts = [p for p in re.split(r"-", spaces_txt) if p.isdigit()]
    if not parts:
        return line
    xnums = [int(mm.group(1)) for mm in X_SPACE_RE.finditer(line)]
    if not xnums:
        return line
    keep = [p for p in parts if int(p) not in set(xnums)]
    if not keep:
        return line
    rebuilt = "Sp." + "-".join(keep)
    return SPACE_RANGE_RE.sub(rebuilt, line, count=1)


def property_sweep_from_text(raw_text: str, max_lines: int = 10) -> List[str]:
    """v67: scan raw text for garden+plot/colon patterns to recover missed property lines."""
    out = []
    for ln in split_lines(raw_text or ''):
        if len(out) >= max_lines:
            break
        if ((GARDEN_NORM_RE.search(normalize_section_name(ln)) if GARDEN_NORM_RE else GARDEN_RE.search(ln)) and (PLOT_CODE_RE.search(ln) or COLON_LOC_RE.search(ln))):
            out.append(ln)
    return out
...
def process_page(pdf_path: str, page_index: int, dpi: int, target_char: Optional[str], reader: Optional[PdfReader] = None, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_sections: Optional[List[str]] = None, facts_mode: str = '', alt_ocr: str = "ocrmac_then_kraken") -> Tuple[Dict, List[Dict], bool]:
    ...
    if text_layer_usable(pdf_text):
        ...
        if ok:
            ...
            # v67: fallback property sweep when no property lines were captured
            try:
                if (not is_interment) and (not any(it.get("Include") and it.get("IsProperty") for it in (items or []))):
                    for ln in property_sweep_from_text(txt):
                        it = item_dict_from_line(ln, struck=False)
                        it["LineSource"] = "fallback_text_scan"
                        items.append(it)
            except Exception:
                pass
            # v67: conditional OSD/deskew + normalized header OCR salvage (only when needed)
            try:
...
def process_dataset(pdf_path: str, out_path: str, dpi: int = 300, kraken_model: str = '', kraken_bin: str = 'kraken', kraken_python: str = '', allow_livetext: bool = True, facts_path: str = '', alt_ocr: str = "ocrmac_then_kraken"):
    ...
    # v68: default garden token initialization
    try:
        init_garden_tokens_from_facts(None)
    except Exception:
        pass
    facts_sections = []
    if facts_path:
    ...
    # v68: initialize garden tokens from FaCTS Section list (improves property detection)
    try:
        init_garden_tokens_from_facts(facts_sections if facts_mode == 'loaded' else None)
    except Exception:
        init_garden_tokens_from_facts(None)
    ...
    # v67: write snapshots next to the input PDF folder
    ...
    for p in tqdm(range(page_count), desc=f"Scanning {filename}", unit="page"):
        ...
        try:
            ocr_health_rows.append({
                "Page": p+1,
                "TextSource": owner_data.get("TextSource",""),
                "OCR_NormReason": owner_data.get("OCR_NormReason",""),
                "OCR_OSD_Applied": owner_data.get("OCR_OSD_Applied", False),
                "OCR_OSD_Rotate": owner_data.get("OCR_OSD_Rotate", 0),
                "OCR_OSD_Conf": owner_data.get("OCR_OSD_Conf", 0),
                "OCR_DeskewApplied": owner_data.get("OCR_DeskewApplied", False),
                "OCR_DeskewAngle": owner_data.get("OCR_DeskewAngle", 0.0),
                "NeedsReview": owner_data.get("NeedsReview", False),
                "NeedsReviewNotes": owner_data.get("NeedsReviewNotes", ""),
            })
        except Exception:
            pass
    ...
    with pd.ExcelWriter(tmp_path, engine=engine) as xw:
        ...
        if not phone_ex_safe.empty:
            ...
            # v67 diagnostics sheets
        try:
            if ocr_health_rows:
                pd.DataFrame(ocr_health_rows).to_excel(xw, index=False, sheet_name='OCR_HEALTH')
        except Exception:
            pass
        try:
            if not owners_master.empty:
                no_prop = owners_master[owners_master.get('HasProperty', False) != True].copy()
                if not no_prop.empty:
                    def top_prop_snips(rt):
                        sn = property_sweep_from_text(str(rt) if rt is not None else '', max_lines=10)
                        return ' || '.join(sn)
                    no_prop['PropertyLikeSnips'] = no_prop.get('RawText','').apply(top_prop_snips)
                    keep_cols = [c for c in ['OwnerRecordID','PageNumber','PrimaryOwnerName','Street','City','State','ZIP','TextSource','NeedsReview','NeedsReviewNotes','PropertyLikeSnips','SnapshotFull'] if c in no_prop.columns]
                    no_prop[keep_cols].to_excel(xw, index=False, sheet_name='NO_PROPERTY_REVIEW')
        except Exception:
            pass
